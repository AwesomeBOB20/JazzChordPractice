              ...
              !forceMode && (
                <View style={{ flexDirection: 'row', backgroundColor: t.bg3, borderRadius: 20, padding: 4, borderWidth: 1, borderColor: t.border }}>
                  <TouchableOpacity onPress={() => setTimeout(() => setInputMode('random'), 0)} style={{ paddingHorizontal: 14, paddingVertical: 6, borderRadius: 16, backgroundColor: inputMode === 'random' ? t.accent : 'transparent' }}>
                    <Text style={{ ...TYPE.label, fontWeight: FONT_WEIGHT.bold, color: inputMode === 'random' ? '#fff' : t.txt2 }}>RANDOM</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => setTimeout(() => setInputMode('manual'), 0)} style={{ paddingHorizontal: 14, paddingVertical: 6, borderRadius: 16, backgroundColor: inputMode === 'manual' ? t.accent : 'transparent' }}>
                    <Text style={{ ...TYPE.label, fontWeight: FONT_WEIGHT.bold, color: inputMode === 'manual' ? '#fff' : t.txt2 }}>MANUAL</Text>
                  </TouchableOpacity>
                </View>
              )
            )}
          ...
          {isQuiz ? (
                ...
                <View style={{ gap: 8 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                    ...
                    >
                      ...
                      <Text style={{ ...TYPE.label, fontWeight: FONT_WEIGHT.bold, letterSpacing: 1, color: t.accent }}>ACTIVE VOICING POOL</Text>
                    </TouchableOpacity>
                    <Text style={{ ...TYPE.label, fontWeight: FONT_WEIGHT.bold, color: t.txt3 }}>{availableVoicingOptions.filter(v => activeVoicingTypes.includes(v.key)).length} Selected</Text>
                  </View>
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                    {availableVoicingOptions.map(item => {
                      ...
                      return (
                        ...
                        >
                          <Text style={{ ...TYPE.body, fontWeight: FONT_WEIGHT.bold, color: isActive ? '#fff' : t.txt2 }}>
                            {item.label}
                          </Text>
                        </TouchableOpacity>
                      );
                    ...
                    <View style={{ gap: 8 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                        ...
                        >
                          ...
                          <Text style={{ ...TYPE.label, fontWeight: FONT_WEIGHT.bold, letterSpacing: 1, color: t.accent }}>INVERSIONS</Text>
                        </TouchableOpacity>
                        <Text style={{ ...TYPE.label, fontWeight: FONT_WEIGHT.bold, color: t.txt3 }}>{activeInversions.filter(i => availableInversions.includes(i)).length} Selected</Text>
                      </View>
            ...
            <View style={{ flexDirection: 'row', flex: 1 }}>
              ...
              <View style={{ width: 75, backgroundColor: t.bg3, borderRightWidth: 1, borderColor: t.border, paddingTop: 16 }}>
                <Text style={{ ...TYPE.caption, color: t.accent, textAlign: 'center', marginBottom: 16, letterSpacing: 1 }}>ROOT</Text>
                    ...
                    {ROOTS.map(r => {
                      ...
                      return (
                        ...
                        onPress={() => {
                          ...
                          <Text style={{ ...TYPE.heading, fontWeight: isActive ? FONT_WEIGHT.bold : FONT_WEIGHT.semibold, color: isActive ? '#fff' : t.txt2 }}>{notes[r]}</Text>
                        </TouchableOpacity>
                      )
                    })}
                  </ScrollView>
              ...
              <View style={{ flex: 1, paddingTop: 16, paddingHorizontal: 16 }}>
                <Text style={{ ...TYPE.caption, color: t.accent, marginBottom: 16, letterSpacing: 1 }}>QUALITY</Text>
                <View style={{ flex: 1, position: 'relative' }}>
                  ...
                  >
                    {CHORD_CATEGORIES.map((cat: { label: string, keys: string[] }, catIdx: number) => (
                      <View key={cat.label} style={{ marginBottom: 20 }} onLayout={(e) => qualCatY.current[cat.label] = e.nativeEvent.layout.y}>
                        <Text style={{ ...TYPE.caption, color: t.txt3, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 12 }}>{cat.label}</Text>
        ...
        {executeLabel && (
          <View style={{ borderTopWidth: 1, borderColor: t.border, paddingHorizontal: 16, paddingVertical: 12, paddingBottom: 12 + insets.bottom }}>
            ...
            >
              ...
              <Text style={{ ...TYPE.body, fontWeight: FONT_WEIGHT.bold, color: '#fff', letterSpacing: 1 }}>{executeLabel}</Text>
            </TouchableOpacity>
          </View>
        )}
      </Animated.View>
